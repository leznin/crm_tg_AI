#!/usr/bin/env python3
"""
Final test for the fixed business accounts API
"""
import sys
import os
import json
...
def test_api_final_fixed():
    """Test the final fixed API logic for business accounts."""
    db = SessionLocal()
    try:
        repo = BusinessAccountRepository(db)

        print("=== FINAL FIXED API TEST ===\n")
        ...
        if accounts:
            ...
            print("API Response (Business Account):")
            print(json.dumps(api_response["accounts"][0], indent=2, ensure_ascii=False, default=str))

            print("\nMessage Summary:")
            total_messages = 0
            for chat in api_response["accounts"][0]["chats"]:
                msg_count = len(chat["messages"])
                total_messages += msg_count
                print(f"  Chat '{chat['first_name'] or chat['title'] or 'Unknown'}': {msg_count} messages")

            print(f"\n✅ TOTAL: {len(api_response['accounts'][0]['chats'])} chats, {total_messages} messages")
